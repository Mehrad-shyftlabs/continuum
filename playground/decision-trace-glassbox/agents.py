...
def _full_close_stages(p, tools, te, model) -> list[BaseAgent]:
    ...
    return [
        _agent(f"{p}-ingest",
               ...
               tools, te, model, {"get_close_data"}),
        _agent(f"{p}-reconcile",
               ...
               tools, te, model, {"get_close_data", "get_materiality_policy"}),
        _agent(f"{p}-variance",
               ...
               tools, te, model, {"assess_materiality"}),
        _agent(f"{p}-intercompany",
               ...
               tools, te, model, {"get_intercompany"}),
        _agent(f"{p}-consolidate",
               ...
               tools, te, model, {"compute_consolidation"}),
        _agent(f"{p}-report",
...
def build_sequential(tools, te, model):
    ...
    return SequentialAgent(name="close-sequential", agents=stages,
                           sequential_config=__seq_cfg()), stages
...
def build_supervised(tools, te, model):
    ...
    sup = create_supervised_agent("close-supervised", stages, quality_threshold=0.6,
                                  max_retries=1, supervisor_model=model)
    return sup, stages
...
def build_planning(tools, te, model):
    ...
    planner = create_planner_agent(
        ...
        max_steps=5, planning_model=model)
...
def build_loop(tools, te, model):
    officer = _agent(
        ...
        tools, te, model, {"get_materiality_policy", "assess_materiality", "compute_consolidation"})
    loop = create_loop_agent("close-loop", officer,
...
def build_reflection(tools, te, model):
    ...
    refl = create_reflection_agent(
        ...
        max_reflections=1, reflection_model=model)
    return refl, [drafter]
...
def build_parallel(tools, te, model):
    ...
    ic = _agent(
        ...
        tools, te, model, {"get_intercompany"})
    par = ParallelAgent(
        name="close-parallel", agents=[decider, ic], model=model,
        parallel_config=ParallelConfig(
            ...
            "them.",
            summary_model=model),
        memory_config=_MEM, config=_CFG)
...
def build_scatter(tools, te, model):
    decider = _agent(
        ...
        "End with EXACTLY:\n" + _DECIDE_TAIL,
        tools, te, model, {"get_materiality_policy", "assess_materiality", "compute_consolidation"})
    ic = _agent("scat-intercompany",
                "Call get_intercompany and report the eliminations in one paragraph (context).",
                tools, te, model, {"get_intercompany"})
    sc = create_scatter_agent("close-scatter", [decider, ic],
                              merge_strategy=MergeStrategy.LLM_SUMMARIZE, split_model=model)
...
def build_router(tools, te, model):
    ...
    router = RouterAgent(
        name="close-router",
        routes=[
            Route(agent_name="lenient-close", description="standard firm policy close", condition="lenient"),
            Route(agent_name="strict-close", description="strict conservative SOX close", condition="strict"),
        ],
...
def build_debate(tools, te, model):
    pro = _agent("close-debate-pro",
                 "You argue the close is materially FINE under the firm's $5M threshold "
                 "(small items are immaterial). 2-3 sentences.", tools, te, model)
    con = _agent("close-debate-con",
                 "You argue the close has a CONTROL ISSUE: the $2M D1 misstatement affects the "
                 "balance and must not be waived. 2-3 sentences.", tools, te, model)
    judge = _agent(
        "close-debate-judge",
...
def build_handoff(tools, te, model):
    controller = _agent(
        ...
        tools, te, model, {"get_materiality_policy", "compute_consolidation"})
    officer = BaseAgent(
        ...
        model=model, tools=_tools(tools, {"get_close_data"}), tool_executor=te,
        handoffs=[Handoff(target_agent="ho-controller", description="Escalate for the final call.",
    ...
    triage = BaseAgent(
        ...
        model=model,
        handoffs=[Handoff(target_agent="ho-close-officer", description="Route to the officer.",
                          return_to_parent=False)],
        memory_config=_MEM, config=_CFG)
